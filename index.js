$(function() {
    ...
    $("#header-icon, #header-rec-btn").on("click", function() {
        $('html, body').animate({ scrollTop: 0 }, 500);
    });
    //ヘッダーアイコンをクリックすると、ページ最上部まで自動スクロールされる処理。
    ...
    $("#header2-reservation-btn").on("click", function() {
        $("html,body").animate({ scrollTop: $('#main-reservation').offset().top });
    });
    $("#header2-system-btn").on("click", function() {
        $("html,body").animate({ scrollTop: $('#main-system').offset().top });
    });
    $("#header2-rental-btn").on("click", function() {
        $("html,body").animate({ scrollTop: $('#main-rental').offset().top });
    });
    $("#header2-artist-btn").on("click", function() {
        $("html,body").animate({ scrollTop: $('#main-artist').offset().top });
    });
    //第２ヘッダーの各ボタンをクリックすると、指定の位置までページスライダーアニメーションを行う処理。
    ...
    //レコーディングについての事前準備をクリックした時にモーダルを表示する処理。
    $('.reservation-link-btn').on('click', function() {
        $('#reservation-pop').addClass('popup-content-active')
    });
    //レコーディングについての事前準備をクリックした時にモーダルを表示する処理。

    //レコーディングについての詳細、機材名・レンタル楽器及び機材の詳細をクリックした時に、背景スクロールができないように固定する処理。
    var scrollPosition;
    $(".rec-detail-link-open, .system-link-open, .rental-link-open, .reservation-link-open").on("click", function() {
